...
class CustomRobot: Robot {
  ...
  func moveToEnemyPosition(enemyPosition: CGPoint) {
    cancelActiveAction()
    
    if enemyPosition.x < position().x {
      turnRobotLeft(Int(angleBetweenHeadingDirectionAndWorldPosition(enemyPosition)))
      moveAhead(100)
    } else {
      turnRobotRight(Int(angleBetweenHeadingDirectionAndWorldPosition(enemyPosition)))
      moveAhead(100)
    }
  }
